...
func applyCreateCommonFields(ctx context.Context, opts CommonFieldAutoFillOptions, entity any) {
	if shouldSkipCommonFields(opts, entity) {
		return
	}
	opts = opts.Normalize()
	now := opts.TimeProvider(ctx)
...
func applySaveCommonFields(ctx context.Context, opts CommonFieldAutoFillOptions, entity any) {
	if shouldSkipCommonFields(opts, entity) {
		return
	}
	opts = opts.Normalize()
...
func applyUpdateCommonFields(ctx context.Context, opts CommonFieldAutoFillOptions, entity any) {
	if shouldSkipCommonFields(opts, entity) {
		return
	}
	opts = opts.Normalize()
	...
	if managed, ok := entity.(dbspi.UpdateTimeAccessor); ok && (opts.OverwriteExplicitValues || managed.GetMtime() == 0) {
		managed.SetMtime(opts.TimeProvider(ctx))
	}
...
func applyCreateCommonFieldsToSlice[T any](ctx context.Context, opts CommonFieldAutoFillOptions, entities []T) {
	for _, entity := range entities {
		applyCreateCommonFields(ctx, opts, entity)
	}
}

func applySaveCommonFieldsToSlice[T any](ctx context.Context, opts CommonFieldAutoFillOptions, entities []T) {
	for _, entity := range entities {
		applySaveCommonFields(ctx, opts, entity)
	}
}

func applyUpdateCommonFieldsToUpdater(ctx context.Context, opts CommonFieldAutoFillOptions, model any, updater dbspi.Updater) {
	...
	if managed, ok := model.(dbspi.UpdateTimeAccessor); ok {
		...
		if opts.OverwriteExplicitValues || !hasUpdateParam(params, fieldName) {
			updater.Set(NewColumn(fieldName), opts.TimeProvider(ctx))
		}
	}
...
func shouldSkipCommonFields(opts CommonFieldAutoFillOptions, entity any) bool {
